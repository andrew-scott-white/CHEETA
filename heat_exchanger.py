...
from gpkit import Model, Variable, units, SignomialsEnabled, Vectorize
...
class HeatExchanger(Model):
    ...
    def performance(self, flight_state):
        return HeatExchangerPerformance(self, flight_state)
...
if __name__ == '__main__':
    ...
    segment = ['Take-off', 'Top-of-Climb', 'Cruise']
    
    for nn in range(len(segment)):
        if nn == 0:
            ii = 0
            N = 6
        if nn == 1:
        ...
        # Cruise Range
        T1 = T[ii] * (1 + 0.5*(gamma-1)*M[ii]**2) # Ambient Stagnation Temperature, K
        
        if nn == 0:
            T_r_low = 0.1*T1 + T1 # correpsonds to T_r = 340 K
            T_r_high = 0.6*T1 + T1 # corresponds to T_r = 500 K
        else:
            T_r_low = 0.5*T1 + T1 # correpsonds to T_r = 360 K
            T_r_high = 1*T1 + T1 # correpsonds to T_r = 360 K
        
        T_r = np.linspace(T_r_low, T_r_high, num=30) # K
        
        flight_state = FlightState(C_D_fuse, C_D_wing)
        flight_state.substitutions.update({
        flight_state['M_\infty']:           M[ii], 
        flight_state['P_{t\\infty}']:       P[ii] * (1 + 0.5*(gamma-1)*M[ii]**2)**(gamma/(gamma-1)) * units('Pa'),
        flight_state['P_\\infty']:          P[ii]*units('Pa'),
        flight_state['T_{t\\infty}']:       T[ii] * (1 + 0.5*(gamma-1)*M[ii]**2) * units('K'),
        flight_state['T_\\infty']:          T[ii]*units('K'),
        flight_state['V_\\infty']:          M[ii]*(gamma*R1*T[ii]*units('K'))**0.5,
        flight_state['rho_\infty']:         rho[ii]*units('kg/m**3'),
        flight_state['C_p_\infty']:         Cp[ii]*units('J/kg/K'),
        flight_state['C_{d,wing}']:         C_D_wing[ii],
        flight_state['C_{d,fuse}']:         C_D_fuse[ii],
        flight_state['\mu_\infty']:         mu[ii],
        })
        
        
        X = np.zeros([len(T_r),len(VR)])
        Y = np.zeros([len(T_r),len(VR)])
        F_net = np.zeros([len(T_r),len(VR)])
        Q = np.zeros([len(T_r),len(VR)])
        V_infty = np.zeros([len(T_r),len(VR)])
        delT = np.zeros([len(T_r),len(VR)])
        eff = np.zeros([len(T_r),len(VR)])
        ...
        for kk in range(len(VR)):
            ...
            for jj in range(len(T_r)):
                HEXPerf.substitutions.update({
                 'T_{h,in}': T_r[jj]*units('K'),         
                 })
                 
                HEXPerf.append(HEXPerf['D_{HEX}'] >= HEXPerf['D_{core}'] + HEXPerf['D_{Nacelle}'] + HEX['m_{HEX}']*g/18.4,)
                ...
                F_net[jj,kk] = np.asarray(F_HEX - D_nac - D_core)
    
                # other HEX parameters
                Q[jj,kk] = np.asarray(sol['variables']['Q_{HEX}'])
                V_infty[jj,kk] = np.asarray(sol['variables']['V_\\infty'])
                eff[jj,kk] = np.asarray(sol['variables']['\epsilon'])
                
                # storing parameters for plotting
                X[jj,kk] = np.asarray(sol['variables']['VR'])
                delT[jj,kk] = T_r[jj] - T1
                Y[jj,kk] = delT[jj,kk]/T1
    
        Z = F_net*V_infty/(Q)
    
        delT_low = (T_r_low - T1)/(T1)
        delT_high = (T_r_high - T1)/(T1)
        ...
        plt.yticks(np.linspace(delT_low, delT_high,num=N))
