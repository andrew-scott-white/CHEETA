...
class FullyElectricBatHybrid(Model):
    ...
    def setup(self, N_p):
        ...
        N_b = Variable('N_b', '-', 'number of batteries')
        ...
        constraints = [
            ...
            m_propsys >= N_s*(stack['m_{fc}'] + HEX['m_{HEX}']) + N_p*(motor['m_{motor}'] + fan['m_{fan}']) + N_b*bat['m_{bat}'], # propulsion system weight
    ...
    def performance(self, flight_state, h_fuel, D_p_fuse, D_p_wing, BLI):
        return FullyElectricBatHybridPerformance(self, flight_state, h_fuel, D_p_fuse, D_p_wing, BLI)

    
class FullyElectricBatHybridPerformance(Model):
    def setup(self, fully_electric, flight_state, h_fuel, D_p_fuse, D_p_wing, BLI):
        ...
        bat_perf = self.bat_perf = fully_electric.bat.performance(flight_state)
        ...
        with SignomialsEnabled():
            constraints += [
            fully_electric['N_s']*stack_perf['P_{out}'] + fully_electric['N_b']*bat_perf['P_{out}'] >= fully_electric['N_p']*motor_perf['P_{in}'],
            ]
