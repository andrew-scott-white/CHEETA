...
class FuelCell(Model):
    ...
    def performance(self, flight_state, h_fuel):
        return FuelCellPerformance(self, flight_state, h_fuel)

class FuelCellPerformance(Model):
    """A component performance model of the fuel cell.
    """
    def setup(self, fuelcell, flight_state_fc, h_fuel):
        ...
        HV = Variable('HV_{fc}', h_fuel, 'MJ/kg', 'fuel heating value of fuel')
        ...
        Q = Variable('Q_{fc}', 'kW', 'heat rejected by the fuel cell')
        ...
        eta_fc = Variable('eta_{fc}', '-', 'fuel cell efficiency')
        eta_net = Variable('eta_{net}', '-', 'net power system efficiency')
        ...
        i_d = Variable('(i/A)', 'A/cm**2', 'fuel cell internal current density')
        ...
        constraints = [
           ...
           # ideal gas usage current
               i_n == 2*F*mdot_fuel/2/.0010079/units('kg')/fuelcell['N_{cells}'], 
               i == i_n*mu_f, # fuel utilization
            
            # Higher-temp GPfit for Nernst (180°C)
...
if __name__ == '__main__':
        ...
        fcPerf = FuelCellPerformance(fc, flight_state_fc, 120)
        fcPerf.substitutions.update({
        
        # Inputs       
        
             #'P_{gross}':       1000        *units('kW'),
             'P_{out}':                 760*units('kW'),
        ...
        # Design inputs
            ...
            '(P/m)_{fc}':           2000*units('W/kg'), # CHEETA Data Sheet
            'A':                    446*units('cm**2'),
            'N_{cells}':            1080,
            'S':                    1.5,
