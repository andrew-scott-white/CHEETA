...
ac.substitutions.update({
    'f_{empty}': 0.554  , # assumed for hydrogen aircraft
    'b_{fuse}': 14*units('ft'),
...
init = {
    ...
    mission.fs.performance.prop.fan_perf['F_{fan}']:       100000*units('kN'),
    mission['F_{net}']:                                         44*units('kN'),
    }

if BLI == 'on':
    sol = model.localsolve(x0 = init)
else:
    sol = model.solve(x0 = init)
print(sol.summary())
...
print("m_{gg} (kg): " + str(2*sol['variables']['m_{gg}']))
print("m_{fan} (kg): " + str(2*sol['variables']['m_{fan}']))
print("m_{propsys} (kg): " + str(sol['variables']['m_{propsys}']))
